...
import { SendMessageInput, GetMessagesInput, RespondMessageInput, SearchMessagesInput, CompactThreadInput, CoordinationError, PermissionError } from '../types/index.js';
export class CoordinationMCPServer {
    ...
    async handleSendMessage(args) {
        ...
        const canSend = await this.participantRegistry.canSendMessage(this.config.participant_id, input.to);
        if (!canSend) {
    ...
    async handleRespondMessage(args) {
    ...
    async handleCompactThread(args) {
    ...
    async handleArchiveResolved(args) {
    ...
    async handleGetStats(args) {
        const participant = args?.participant || this.config.participant_id;
    ...
    async handleRegisterParticipant(args) {
        const input = args;
    ...
    async handleSetupGuide(args) {
        const topic = args?.topic || 'quickstart';
        const guides = {
