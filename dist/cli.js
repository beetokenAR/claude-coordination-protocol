...
program
    ...
    try {
        ...
        const input = {
            to: options.to.split(',').map((p) => p.trim()),
            type: options.type,
...
participant
    ...
    try {
        ...
        if (options.priority) {
            updates.default_priority = options.priority;
        }
        if (options.status) {
            updates.status = options.status;
        }
        await participantRegistry.updateParticipant(id, updates, config.participant_id);
...
participant
    ...
    try {
        ...
        const db = new CoordinationDatabase(config.data_directory);
        const participantRegistry = new ParticipantRegistry(db, config.data_directory);
        await participantRegistry.deactivateParticipant(id, config.participant_id);
