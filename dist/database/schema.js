...
export function initializeSchema(db) {
    try {
      ...
      VALUES (?, ?, ?)
    `);
        upsertMetadata.run('schema_version', SCHEMA_VERSION.toString(), new Date().toISOString());
    }
    catch (error) {
        throw new DatabaseError(`Failed to initialize database schema: ${error.message}`, { error: error.message });
    }
}
/**
...
export function checkSchemaVersion(db) {
    try {
        ...
        const result = getVersion.get('schema_version');
        if (!result) {
