 ...
 */
export function safeParseInput(schema, data, defaultValue) {
...
export function validateParticipantId(id) {
    ...
    if (!/^@[a-zA-Z][a-zA-Z0-9_-]*$/.test(id)) {
        throw new ValidationError(`Participant ID contains invalid characters. Must be @followed by alphanumeric, underscore, or dash: ${id}`);
    }
}
/**
...
export function validateMessageId(id) {
    if (!/^[A-Z]+-\d{3}(-[A-Z]+-\d{3})*$/.test(id)) {
        throw new ValidationError(`Message ID must follow format TYPE-NNN (e.g., CONTRACT-001): ${id}`);
    }
}
/**
...
export function validateThreadId(id) {
    if (!/^[A-Z]+-\d{3}-thread(-\w+)?$/.test(id)) {
        throw new ValidationError(`Thread ID must follow format TYPE-NNN-thread or TYPE-NNN-thread-suffix: ${id}`);
    }
}
/**
 ...
 */
export function validateContentSize(content, maxTokens = 10000) {
    // Rough token estimation: ~4 characters per token
    const estimatedTokens = Math.ceil(content.length / 4);
    if (estimatedTokens > maxTokens) {
        throw new ValidationError(`Content too large: ~${estimatedTokens} tokens (max: ${maxTokens}). ` +
 ...
 */
export function validateArraySize(array, maxSize, name) {
 ...
 */
export function validateNoCycles(messageId, dependencies, getDependencies) {
    ...
    if (hasCycle(messageId)) {
        throw new ValidationError(`Circular dependency detected involving message ${messageId}`, { messageId, dependencies });
    }
}
