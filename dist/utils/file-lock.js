...
export class FileLock {
    ...
    acquired = false;
    lockHandle;
    constructor(dataDir, lockName = 'coordination.lock') {
        const lockDir = path.join(dataDir, 'locks');
        this.lockPath = path.join(lockDir, lockName);
    }
    async acquire(maxRetries = 50, retryDelay = 100) {
        if (this.acquired) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                throw new CoordinationError(`Failed to acquire file lock: ${error.message}`, 'LOCK_ACQUISITION_FAILED', { error: error.message, attempt, lockPath: this.lockPath });
            }
        }
        throw new CoordinationError(`Could not acquire file lock after ${maxRetries} attempts`, 'LOCK_TIMEOUT', { maxRetries, lockPath: this.lockPath });
    }
    async release() {
    ...
    async forceRelease() {
        try {
            ...
            if (error.code !== 'ENOENT') {
                throw new CoordinationError(`Failed to force release lock: ${error.message}`, 'LOCK_FORCE_RELEASE_FAILED', { error: error.message, lockPath: this.lockPath });
            }
        }
    }
    async isStalelock() {
 ...
 */
export async function withFileLock(dataDir, operation, lockName = 'coordination.lock') {
