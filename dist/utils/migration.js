...
export class LegacyMigrationTool {
     ...
     */
    async migrateFromCoordinationFile(filePath) {
        try {
            ...
            throw new DatabaseError(`Failed to migrate from ${filePath}: ${error.message}`, { filePath, error: error.message });
        }
    }
    /**
            ...
            if (messageMatch) {
                ...
                for (let j = i + 1; j < Math.min(i + 5, lines.length); j++) {
                    ...
                    if (fromMatch) {
                        ...
                        toParticipant = fromMatch[2];
                        priority = fromMatch[3];
                    }
     ...
     */
    async importMessages(legacyMessages, sourceDir) {
        const result = {
        ...
        for (const legacyMsg of legacyMessages) {
            try {
                ...
                const existing = await this.messageManager.getMessageById(legacyMsg.id, legacyMsg.from, 'summary');
                if (existing) {
                ...
                const importedMessage = await this.messageManager.createMessage(messageInput, legacyMsg.from);
    ...
    inferMessageDate(content, messageId) {
    ...
    inferCapabilities(participantId) {
    ...
    async updateMessageTimestamp(messageId, timestamp) {
        const updateTimestamp = this.db.prepare(`
      UPDATE messages SET created_at = ?, updated_at = ? WHERE id = ?
    `);
        updateTimestamp.run(timestamp.toISOString(), timestamp.toISOString(), messageId);
    }
    async updateMessageStatus(messageId, status) {
        const updateStatus = this.db.prepare(`
      UPDATE messages SET status = ?, updated_at = ? WHERE id = ?
    `);
        updateStatus.run(status, new Date().toISOString(), messageId);
    }
    generateMigrationSummary(result) {
        const lines = [
            `🔄 Migration completed!`,
            ``,
            `📊 **Summary:**`,
     ...
     */
    async exportToLegacyFormat(outputPath) {
        try {
            const messages = await this.messageManager.getMessages({
                limit: 1000,
                detail_level: 'summary'
            }, '@system'); // System can see all messages
            const lines = [
            ...
            throw new DatabaseError(`Failed to export to ${outputPath}: ${error.message}`, { outputPath, error: error.message });
        }
    }
}
