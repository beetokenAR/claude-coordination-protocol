...
describe('IndexingEngine', () => {
  ...
  describe('searchMessages', () => {
    ...
    it('should find messages by full-text search', async () => {
      ...
      const authMessages = results.filter(
        r =>
    ...
    it('should search by tags', async () => {
      ...
      // Should find at least one result that matches our test message
      expect(results.length).toBeGreaterThanOrEqual(1)

      // Find our specific test message
      const testMessage = results.find(r => r.message.subject === 'API Authentication Contract')
      expect(testMessage).toBeDefined()
      expect(testMessage!.message.tags).toContain('api')
      expect(testMessage!.message.tags).toContain('auth')

      // All returned messages should have both tags
      results.forEach(result => {
        expect(result.message.tags).toContain('api')
        expect(result.message.tags).toContain('auth')
      })
    })
      ...
      const message = await messageManager.createMessage(
        {
          ...
          content:
            'Found a security vulnerability in the authentication endpoint that needs immediate fixing.',
          response_required: true,
          expires_in_hours: 6,
          tags: ['security'],
        },
  ...
  describe('findRelatedMessages', () => {
    ...
    it('should find related messages based on content similarity', async () => {
      ...
      const authMessage = relatedMessages.find(r => r.message.subject.includes('Authentication'))
      expect(authMessage).toBeDefined()

      // Results should have relevance scores (can be 0 if no similarity found, but should be valid numbers)
      for (const result of relatedMessages) {
        ...
        expect(isNaN(result.relevance_score)).toBe(false)
      }
    })
