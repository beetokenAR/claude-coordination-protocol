...
export class SecureParticipantRepository implements IParticipantRepository {
   ...
   */
  async findById(id: ParticipantId): Promise<SecureParticipant | null> {
    ...
    const row = stmt.get({ id: id.value }) as ParticipantRow | undefined
   ...
   */
  async save(participant: SecureParticipant): Promise<void> {
   ...
   */
  async deleteById(id: ParticipantId): Promise<boolean> {
    const stmt = this.db.prepare(`
   ...
   */
  async findActive(): Promise<SecureParticipant[]> {
    const stmt = this.db.prepare(`
      SELECT id, capabilities, security_level, last_seen, is_active, 
             failed_attempts, locked_until, created_at, updated_at
      FROM secure_participants 
    ...
    const rows = stmt.all() as ParticipantRow[]
   ...
   */
  async findBySecurityLevel(level: string): Promise<SecureParticipant[]> {
    // Validar que el nivel sea válido
    if (!Object.values(SecurityLevel).includes(level as SecurityLevel)) {
    ...
    const rows = stmt.all({ level }) as ParticipantRow[]
    return rows.map(row => this.mapRowToParticipant(row))
  }
   ...
   */
  async findLocked(): Promise<SecureParticipant[]> {
    const stmt = this.db.prepare(`
      SELECT id, capabilities, security_level, last_seen, is_active, 
             failed_attempts, locked_until, created_at, updated_at
      FROM secure_participants 
   ...
   */
  private async create(participant: SecureParticipant): Promise<void> {
    const stmt = this.db.prepare(`
   ...
   */
  private async update(participant: SecureParticipant): Promise<void> {
    const stmt = this.db.prepare(`
      UPDATE secure_participants 
      SET 
   ...
   */
  private mapRowToParticipant(row: ParticipantRow): SecureParticipant {
    const id = ParticipantId.create(row.id)
    const capabilities = JSON.parse(row.capabilities) as string[]
    const securityLevel = row.security_level as SecurityLevel
    ...
    return SecureParticipant.reconstitute(
      id,
      capabilities,
      securityLevel,
      lastSeen,
   ...
   */
  async findByCapability(capability: string): Promise<SecureParticipant[]> {
    ...
    const rows = stmt.all({ capability: sanitizedCapability }) as ParticipantRow[]
    return rows.map(row => this.mapRowToParticipant(row))
  }
