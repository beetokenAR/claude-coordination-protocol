...
export class CompactionEngine {
  ...
  // Prepared statements
  private selectThreadMessages: any
  private updateMessageStatus: any
  private insertCompactedMessage: any
  private selectConversation: any
  private updateConversation: any
  ...
  private async summarizeThread(
    ...
    const compactedMessage = {
      ...
      summary,
      content_ref: await this.storeCompactedContent(
    ...
    this.db.transaction(() => {
      ...
      for (const msg of consolidatedMessages) {
        this.insertCompactedMessage.run(
          msg.id, msg.thread_id, msg.from, JSON.stringify(msg.to),
          msg.type, msg.priority, msg.status, msg.subject, msg.summary,
          msg.content_ref, msg.created_at.toISOString(), msg.updated_at.toISOString(),
          msg.expires_at?.toISOString() || null, msg.response_required ? 1 : 0,
          JSON.stringify(msg.dependencies), JSON.stringify(msg.tags), null
        )
      }
    })
  ...
  private mergeSummaries(messages: any[]): string {
    ...
    const summaries = messages.map((msg, index) => 
