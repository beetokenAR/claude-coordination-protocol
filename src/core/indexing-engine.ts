...
export class IndexingEngine {
  ...
  private searchMessagesFTS: Statement
  private searchMessagesByTags: Statement
  private searchMessagesByDateRange: Statement
  private updateMessageTags: Statement
  ...
  async indexMessage(message: CoordinationMessage): Promise<void> {
    try {
      ...
      throw new DatabaseError(`Failed to index message: ${error.message}`, {
  ...
  async getTagSuggestions(
    ...
    try {
      ...
      `
        ...
        .all(requestingParticipant, requestingParticipant, query, limit) as Array<{
        tag: string
        usage_count: number
      }>
      ...
      throw new DatabaseError(`Failed to get tag suggestions: ${error.message}`, {
        query,
        error: error.message,
      })
    }
  ...
  async getMessageStats(
    ...
    try {
      ...
      `
        ...
        .all(participantId, participantId, since.toISOString()) as Array<{
        type: string
        count: number
      }>

      // Get distribution by priority
      const byPriority = this.db
        .prepare(
          `
      ...
      const responseRate =
        responseStats.total_requiring_response > 0
      ...
      throw new DatabaseError(`Failed to get message statistics: ${error.message}`, {
        participantId,
        error: error.message,
      })
    }
  ...
  async findRelatedMessages(
    ...
    try {
      ...
      throw new DatabaseError(`Failed to find related messages: ${error.message}`, {
        messageId,
        error: error.message,
      })
    }
